...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const token = (await cookies()).get("team-token")?.value
    ...
    // Check if task is assigned to the team member
    ...
    const teamMember = await TeamMember.findOne({ email: decoded.email })
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="padding: 20px;">
            ...
            <p>Hello Admin,</p>
            <p>${teamMember?.name || decoded.email} has updated the status of a task:</p>
